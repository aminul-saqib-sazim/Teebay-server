...
export class Migration20260119075048_create_products_table extends Migration {
  ...
  async down(): Promise<void> {
    this.addSql(
      'create table "permissions" ("id" serial primary key, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "name" text check ("name" in (\'CREATE_USER\', \'READ_USER\', \'UPDATE_USER\', \'DELETE_USER\')) not null);',
    );

    this.addSql(
      'create table "roles" ("id" serial primary key, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "name" text check ("name" in (\'SUPER_USER\', \'ADMIN\')) not null);',
    );
    ...
    this.addSql(
      'create table "user_profiles" ("id" serial primary key, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "first_name" varchar(255) not null, "last_name" varchar(255) not null, "user_id" int4 not null, "role_id" int4 not null);',
    );
    this.addSql(
    ...
    this.addSql(
      'alter table "user_profiles" add constraint "user_profiles_role_id_foreign" foreign key ("role_id") references "roles" ("id") on update cascade on delete no action;',
    );
