...
export class Migration20260201143124_create_orders_table extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'alter table "orders" add constraint "orders_product_id_foreign" foreign key ("product_id") references "products" ("id") on update cascade;',
    );
    this.addSql(
      'alter table "orders" add constraint "orders_buyer_id_foreign" foreign key ("buyer_id") references "users" ("id") on update cascade;',
    );

    this.addSql('alter table "products" alter column "rental_price" drop default;');
    this.addSql(
      'alter table "products" alter column "rental_price" type numeric(10,2) using ("rental_price"::numeric(10,2));',
    );
  }
